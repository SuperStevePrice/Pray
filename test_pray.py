#!/usr/bin/env python3
"""
test_pray.py — Test suite for pray.py.
...
Usage:
    python3 test_pray.py                    # test all prayers
    python3 test_pray.py --prayer ave       # test Ave Maria only
    python3 test_pray.py --prayer lords     # test Lord's Prayer only
    python3 test_pray.py --dry-run          # show what would be tested, no audio
"""
...
def main() -> None:
    ...
    log_path   = LOGS_DIR / f"test_pray_{timestamp}.log"
    ...
    header = (
        f"test_pray.py — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"{'─' * 60}\n"
    ...
    for prayer_key, prayer in prayers_to_test.items():
        ...
        for language in prayer.language_names():
            # For Polish, only Zosia is a real voice — others alias to Zosia
            # Test each unique voice string once per language
